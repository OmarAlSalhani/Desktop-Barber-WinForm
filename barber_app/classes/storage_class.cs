...
namespace barber_app.classes
{
    class storage_class
    {
        ...
        public static void storage_log_add(string event_name, double rsed, int storage_id)
        {
            ...
            SQLiteCommand command = new SQLiteCommand("insert into storage_logs_table (storage_event,the_rsed,the_date,the_time,user_id,storage_id) values(@storage_event,@the_rsed,@the_date,@the_time,@user_id,@storage_id)", connection_class.connection());
        ...
        {
            connection_class.connection();
            SQLiteCommand command = new SQLiteCommand("update storage_table set storage_value=storage_value+@storage_value where id=@storage_id", connection_class.connection());
        ...
        {
            DataTable table = connection_class.select($"select ifnull(storage_value,0) from storage_table where id={settings_files.main_settings.Default.storage_id}");
